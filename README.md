<table>
<tr>
<td width="66%" valign="top">

### My GitHub Activity Pet
Esta mascota vive del código: duerme, despierta y se pone cada vez
más contenta según los commits que hago **cada día**. A medianoche
vuelve a dormirse y empieza de nuevo.

**¿Quieres una para tu propio perfil?**:

1. Copia [`scripts/pet.py`](https://github.com/miguelgomez75/miguelgomez75/blob/main/scripts/pet.py) y [`.github/workflows/pet.yml`](https://github.com/miguelgomez75/miguelgomez75/blob/main/.github/workflows/pet.yml) a tu repo `TU_USUARIO/TU_USUARIO`.
2. Sube tus propios sprites (`.gif`/`.png`/`.jpg`) a `assets/sprites/`, con los nombres de cada estado.
3. En **Settings → Actions → General**, activa *"Read and write permissions"*.
4. Ejecuta el workflow una vez desde la pestaña **Actions**.
5. Pega la tabla de tu README apuntando a `dist/pet.gif` y `dist/pet-status-*.svg`.

</td>
<td width="33%" valign="top" align="center">

<img src="https://raw.githubusercontent.com/miguelgomez75/miguelgomez75/main/dist/pet.gif" width="150" /><br/>

<img alt="Estado de la mascota" src="https://raw.githubusercontent.com/miguelgomez75/miguelgomez75/main/dist/pet-status-dark.svg" width="230" />

<img alt="historial de la mascota" src="https://raw.githubusercontent.com/miguelgomez75/miguelgomez75/main/dist/pet-stats-dark.svg" width="230"/>

</td>
</tr>
</table>
